    ...
    return (
      <div className="flex-1 flex items-center justify-center min-h-[60vh]">
        <div className="flex flex-col items-center gap-3">
          <Activity size={32} className="text-violet-600 animate-pulse" />
          <p className="text-gray-500 font-medium">Loading live markets...</p>
        </div>
      </div>
    );
  }
    ...
    <div className="flex-1 bg-[#F8FAFC]">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
        ...
        <div className="flex items-center justify-between mb-8">
          ...
          <div className="flex items-center gap-2 text-sm text-green-600 bg-green-50 px-4 py-2 rounded-full border border-green-100 font-medium shadow-sm">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-12">
          {indices.map((index, idx) => {
            ...
            return (
              <div 
                key={idx} 
                className="bg-white rounded-2xl p-6 border border-gray-200 shadow-sm hover:shadow-md transition-shadow relative overflow-hidden group"
              >
                ...
                <div className="flex justify-between items-start mb-4 relative">
                  <div>
                    <h3 className="font-bold text-gray-800 text-lg">{index.label}</h3>
                    <p className="text-xs text-gray-400 mt-1 uppercase tracking-wider font-semibold">{index.symbol}</p>
                  </div>
                ...
                <div className="relative">
                  <div className="text-3xl font-bold text-gray-900 mb-2 font-mono tracking-tight">
                    {index.label === 'USD/INR' ? '₹' : ''}
                    {index.currentPrice?.toLocaleString('en-IN', { minimumFractionDigits: 2 })}
                  </div>
                  <div className={`flex items-center gap-1.5 font-medium ${isPositive ? 'text-green-600' : 'text-red-600'}`}>
                    <span>{isPositive ? '+' : ''}{index.change}</span>
                    <span className="text-sm bg-white/50 px-1.5 py-0.5 rounded backdrop-blur-sm">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          ...
          <div className="bg-white p-8 rounded-3xl shadow-sm border border-gray-200">
            <div className="flex items-center gap-3 mb-6">
              ...
              <h2 className="text-xl font-bold text-gray-800">Global Markets (Coming Soon)</h2>
            </div>
